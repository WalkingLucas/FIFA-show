...
一个 Windows 桌面世界杯比分悬浮窗。窗口常驻最上层，不占用端口，适合边工作、边看视频或边处理其他事情时快速扫一眼赛程和比分。

## 功能

- 无标题栏、无边框、半透明圆角卡片。
- 默认窗口约 `360 x 160`，紧凑模式约 `320 x 104`。
- 永远置顶，不显示在任务栏。
- 支持拖拽移动窗口。
- 右键菜单支持刷新、切换紧凑模式、退出。
...
- 球队名称使用中文名，并保留英文缩写。
- 支持收藏球队，收藏球队相关比赛优先展示。
- 点击未来比赛可做个人预测：主胜、平、客胜。
- 点击直播或已结束比赛可查看单场详情，包括进球、助攻、牌、换人和文字直播摘要。
- 支持统计个人预测命中率。
- 榜单分为进球、助攻、纪律、趣味和预测。
- 不启动本地 Web 服务，不占用端口。
...
`scoreboard` 用于赛程、比分和比赛状态。`summary` 用于单场详情，并派生进球、助攻、黄牌、红牌、犯规等榜单数据。
...
- 停留在直播详情页时：单场详情每 60 秒刷新一次。
- 有直播比赛时会每 60 秒检查进球事件；发现新增进球后立即刷新比分。
- 右键点击“刷新”会立即手动刷新。
- 刷新失败时继续显示上一次缓存结果，并提示缓存更新时间。
...
- 点击球队名称查看球队战绩和赛程。
- 点击未来比赛行进入预测页。
- 点击直播或 FT 比赛行进入详情页，查看进球、助攻等事件。
- 在 `榜 > 预测` 中查看预测数量、已结算数量、猜中次数和命中率。

## 版本管理

- 当前版本见 `package.json`。
- 每个正式版本会在 `CHANGELOG.md` 记录主要变化。
- 版本号按 `主版本.功能版本.修订版本` 管理：功能变化升级中间位，小修复升级最后一位。
